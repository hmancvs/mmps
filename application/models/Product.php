...
class Product extends BaseEntity  {
	...
	public function setTableDefinition() {
		...
		$this->hasColumn('email_minimum', 'integer', null, array('default' => 0));
		$this->hasColumn('subtract', 'integer', null, array('default' => 0));
		$this->hasColumn('shipping', 'integer', null, array('default' => 0));
		$this->hasColumn('showquantity', 'integer', null, array('default' => 0));
		$this->hasColumn('unitprice', 'decimal', 10, array('default' => NULL));
		$this->hasColumn('price', 'decimal', 10, array('notnull' => true, 'notblank' => true, 'default' => NULL));
		$this->hasColumn('currency', 'integer', null);
		...
		$this->hasColumn('dimensionunit', 'integer', null);
		$this->hasColumn('weight', 'decimal', 10, array('default' => NULL));
		$this->hasColumn('weightunit', 'integer', null);
		$this->hasColumn('points', 'decimal', 10, array('default' => NULL));
		$this->hasColumn('dateavailable', 'date');
	...
	public function construct() {
		...
		$this->addCustomErrorMessages(array(
										...
										"categoryid.notblank" => $this->translate->_("product_category_error"),
										"price.notblank" => $this->translate->_("product_price_error")
       	       						));     
	}
	/*
	...
	function hasProfileImage(){
		...
		$real_path = $real_path.$this->getStoreID().DIRECTORY_SEPARATOR."product_".$this->getID().DIRECTORY_SEPARATOR."large_".$this->getProfilePhoto();
	...
	function getRelativeProfilePicturePath(){
		$real_path = APPLICATION_PATH.DIRECTORY_SEPARATOR."..".DIRECTORY_SEPARATOR."public".DIRECTORY_SEPARATOR."uploads".DIRECTORY_SEPARATOR."stores".DIRECTORY_SEPARATOR."store_";
		$real_path = $real_path.$this->getStoreID().DIRECTORY_SEPARATOR."product_".$this->getID().DIRECTORY_SEPARATOR."medium_".$this->getProfilePhoto();
		if(file_exists($real_path) && !isEmptyString($this->getProfilePhoto())){
			return $real_path;
		}
	...
	function getSmallPicturePath() {
		...
		if($this->hasProfileImage()){
			$path = $baseUrl.'/uploads/stores/store_'.$this->getStoreID().'/product_'.$this->getID().'/small_'.$this->getProfilePhoto();
		}
		return $path;
	}
	...
	function getThumbnailPicturePath() {
		...
		if($this->hasProfileImage()){
			$path = $baseUrl.'/uploads/stores/store_'.$this->getStoreID().'/product_'.$this->getID().'/thumbnail_'.$this->getProfilePhoto();
		}
		return $path;
	}
	...
	function getMediumPicturePath() {
		...
		if($this->hasProfileImage()){
			$path = $baseUrl.'/uploads/stores/store_'.$this->getStoreID().'/product_'.$this->getID().'/medium_'.$this->getProfilePhoto();
		}
		return $path;
	}
	...
	function getLargePicturePath() {
		...
		if($this->hasProfileImage()){
			$path = $baseUrl.'/uploads/stores/store_'.$this->getStoreID().'/product_'.$this->getID().'/large_'.$this->getProfilePhoto();
		}
		return $path;
	}
	# fetch next id
